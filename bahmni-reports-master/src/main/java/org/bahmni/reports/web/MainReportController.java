...
public class MainReportController {
    ...
    public void getNewReport(CustomReportParams reportParams, HttpServletResponse response) {
        try {
            converter.applyHttpHeaders(reportParams.getResponseType(), response, reportParams.getName());
            CustomReportGenerator reportGenerator = new CustomReportGenerator(reportParams, response.getOutputStream(), allDatasources, bahmniReportsProperties, httpClient, converter);
            reportGenerator.invoke();
        } catch (Throwable e) {
            catchBlock(response, e);
        }
